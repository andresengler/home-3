...
const resumeData = [/* ... tu mismo array ... */]
          ...
          >
            {['spanish', 'english'].map((lang) => (
              <div key={lang}>
                ...
                <p className="font-sans text-[15px] text-gray-600 dark:text-gray-300 leading-relaxed">
                  {item[lang].role}
                </p>
              </div>
            ))}
        ...
        {section.items.map((item: any, index: number) => (
          <motion.div key={index} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 * (index + 1) }}>
            <Link href={item.href} className="group block">
              <motion.p className="font-sans text-[15px] leading-snug" whileHover={{ x: 2 }} transition={{ duration: 0.2 }}>
...
export default function Resume() {
  ...
  return (
    <div className="relative min-h-screen bg-white dark:bg-black text-gray-800 dark:text-white transition-colors duration-1000">
      <div className="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 py-12 space-y-10">
        <div className="relative">
          {/* Menu lateral */}
          <nav className="hidden lg:block absolute -left-56 top-0">
          ...
          <div>
            ...
            <p className="font-sans text-[15px] text-gray-600 dark:text-gray-300 leading-relaxed mt-4">
